...
bool testSHA512(string? message, char delimiter = '\uffff') {
    bool rc = false;
    try {
...
bool testMD5(string? message, char delimiter = '\uffff') {
    bool rc = false;
    try {
        message = message ?? string.Empty;
        int k1 = message.IndexOf(delimiter);
...
bool testSHA512_SA(string? message, char delimiter = '\uffff') {
    bool rc = false;
    try {
        message = message ?? string.Empty;
        int k1 = message.IndexOf(delimiter);
...
bool testMD5_SA(string? message, char delimiter = '\uffff') {
    bool rc = false;
    try {
        message = message ?? string.Empty;
        int k1 = message.IndexOf(delimiter);
...
byte[] deRSA(string sbhcs, string xmlparam) {
    byte[] rc = { };
    try {
...
bool eqSHA512(string s, byte[] hash) {
    bool rc = true;
    s = s ?? string.Empty;
    byte[] sb = Encoding.ASCII.GetBytes(s);
